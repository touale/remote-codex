...
export async function sessionControls(invoke: Invoke, session: string, workspace: string, remote: string) {
  ...
  await input.setValue('Revise the plan to include a verification step.');
  const messages = await browser.execute(() => document.querySelectorAll('.message.user').length);
  await $('button=Continue planning').click();
  await expect($('.plan-revision')).toBeDisplayed();
  await expect($('[aria-label="Plan next steps"]')).not.toExist();
  await expect(input).toHaveValue('Revise the plan to include a verification step.');
  expect(await browser.execute(() => document.activeElement?.getAttribute('aria-label'))).toBe('Message Codex');
  expect(await browser.execute(() => document.querySelectorAll('.message.user').length)).toBe(messages);
  expect((await invoke<SessionSnapshot>('session_snapshot', { id: session })).settings.mode).toBe('plan');
  await $('button[aria-label="Cancel plan revision"]').click();
  await expect($('.plan-revision')).not.toExist();
  await expect(input).toHaveValue('Revise the plan to include a verification step.');
  await $('button=Continue planning').click();
  await browser.saveScreenshot(new URL('../../../.artifacts/desktop-e2e/plan-revision.png', import.meta.url).pathname);
  await $('button[aria-label="Send plan changes"]').click();
  await expect($('.messages')).toHaveText(expect.stringContaining('Revised workspace plan'));
  await $('.working').waitForExist({ reverse: true });
  await expect($('.plan-revision')).not.toExist();
  await $('button=Continue planning').waitForEnabled();
  const planning = await invoke<SessionSnapshot>('session_snapshot', { id: session });
  await $('button=Clear context and implement').click();
  await expect($('.messages')).toHaveText(expect.stringContaining('Implemented the revised plan in a fresh context.'));
  await $('.working').waitForExist({ reverse: true });
  const fresh = await $('footer').getAttribute('data-session-id');
  expect(fresh).toBeTruthy();
  expect(fresh).not.toBe(session);
  const implementation = await invoke<SessionSnapshot>('session_snapshot', { id: fresh });
  expect(implementation.settings).toEqual({ ...planning.settings, mode: 'agent' });
  const freshHistory = await invoke<HistoryPage>('session_history', { id: fresh, cursor: null });
  expect(freshHistory.turns).toHaveLength(1);
  expect(JSON.stringify(freshHistory)).toContain('Revised workspace plan');
  expect(JSON.stringify(freshHistory)).not.toContain('Make a short plan for this workspace.');
  expect((await invoke<SessionSnapshot>('session_snapshot', { id: session })).settings.mode).toBe('plan');
  await $(`[data-session-id="${session}"] .tree-label`).click();
  await $('button=Implement plan').waitForEnabled();
