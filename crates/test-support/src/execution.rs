//! Bounded execution peer used by native service contract tests.
use crate::ProbeResult as TestResult;
...
pub struct Peer {
    pub reader: codec::Reader<BufReader<OwnedReadHalf>>,
...
impl Peer {
    ...
    pub async fn send(&mut self, operation: &str, message: Value) -> TestResult {
        codec::write(
            &mut self.writer,
            &Frame::Execute {
    ...
    pub async fn call(&mut self, method: &str, params: Value) -> TestResult<Value> {
        self.send(
    ...
    pub async fn response(&mut self) -> TestResult<Value> {
    ...
    pub async fn detach(mut self) -> TestResult<i64> {
